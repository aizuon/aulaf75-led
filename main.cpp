...
{
	HidD_SetFeature(hDevice, INIT_COMMAND_HEX.data(), static_cast<ULONG>(INIT_COMMAND_HEX.size()));
	...
	HidD_SetFeature(hDevice, FINAL_COMMAND_HEX.data(), static_cast<ULONG>(FINAL_COMMAND_HEX.size()));
}
	...
	if (msg == WM_POWERBROADCAST && wParam == PBT_POWERSETTINGCHANGE)
	{
		...
		if (IsEqualGUID(setting->PowerSetting, GUID_CONSOLE_DISPLAY_STATE))
		{
			...
			if (state == 0 && g_ledOn)
			{
				...
				SendLedCommand(g_hDevice, TURN_OFF_COMMAND_HEX);
				g_ledOn = false;
			}
			else if (state == 1 && !g_ledOn)
			{
				std::cout << "Display ON -> Turning LEDs ON" << std::endl;
				SendLedCommand(g_hDevice, TURN_ON_COMMAND_HEX);
				g_ledOn = true;
			}
		}
	}
